...
monthly_stats_fig %>% 
  ggsave(filename = "figs/global_monthly_river_ice_extent.pdf",
         width = 1.5,
         height = 1.5, 
         dpi = "print",
         colormode = "rgb")
...
decadal_change_monthly_obs_bar %>% 
  ggsave(filename = "figs/decadal_change_monthly_obs_bar_global.pdf",
         width = 1.5,
         height = 1.5, 
         dpi = "print",
         colormode = "rgb")
...
world_clipped_nh = world %>% 
  st_intersection(extentsfc_nh)

world_clipped_sh = world %>% 
  st_intersection(extentsfc_sh)

plot(world_clipped_nh)
plot(world_clipped_sh)

st_write(world_clipped_nh, dsn = "~/Google_Drive/monthly_map_river_ice_future_change_global/world_clipped_nh.shp")
st_write(world_clipped_sh, dsn = "~/Google_Drive/monthly_map_river_ice_future_change_global/world_clipped_sh.shp")
```
...
for (j in 1:length(models)) {
  for (i in 1:length(rcps)) {
    for (k in 1:length(ns)) {
      ...
      if (ns[k] == "nh") {
        world_clipped = world_clipped_nh
        crs = 3995
        xlim = c(-7e06, 8e06)
        ylim = c(-6e06, 7e06)
        ...
        world_clipped = world_clipped_sh
        crs = 3031
        xlim = c(-7.5e6, 7.5e6)
        ylim = c(-7.5e6, 7.5e6)
      ...
      if (ns[k] == "nh") {
        ...
        monthly_diffs = monthly_diffs +
          geom_text(data = stats, aes(x = x, y = y, label = paste0(round(diff21 * 100, digits = 2), "\n(", round(perc_change * 100, digits = 0), "%)"), fontface = "bold"), size = 8 * 5 / 14, color = "red", hjust = "middle") +
          scale_fill_gradient2(
            low = "red",
            high = "blue",
            mid = "grey",
      ...
      monthly_diffs %>%
        ggsave(filename = paste0("figs/predicted_ice_extent_map_monthly_", models[j], "_", rcps[i], "_", ns[k], ".png"),
               width = 6,
               height = 6,
               dpi = "retina")
    }
...
for (j in 1:length(models)) {
  for (i in 1:length(rcps)) {
    for (k in 1:length(ns)) {
      ...
      duration_comps %>% 
        ggsave(filename = paste0("figs/ice_duration_map_comparison_", models[j], "_", rcps[i], "_", ns[k], ".png"),
               width = 6,
               height = 3.5)
    }
...
for (j in 1:length(models)) {
  for (i in 1:length(rcps)) {
    for (k in 1:length(ns)) {
      ...
      duration_diff %>% 
        ggsave(filename = paste0("figs/ice_duration_map_difference_", models[j], "_", rcps[i], "_", ns[k], ".png"),
               width = 3,
               height = 3.5)
    }
...
predicted_ice_affected_percent_global_temp %>% 
  ggsave(
    filename = "figs/predicted_ice_affected_percent_global_delta_gsat.png",
    width = 3,
    height = 3.5
  )
...
predicted_ice_duration_global_temp %>% 
  ggsave(
    filename = "figs/predicted_ice_duration_global_delta_gsat.png",
    width = 3,
    height = 3.5
  )
