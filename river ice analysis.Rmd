...
model_comp %>% 
  ggsave(filename = "figs/final_figures/fig2b_ylabel_corrected.pdf",
         device = "pdf",
         units = "mm",
         width = 89,
         height = 45,
...
plot(world_clipped_nh_land %>% st_transform(crs = 3995))
...
for (j in 1:length(models)) {
  for (i in 1:length(rcps)) {
    for (k in 1:length(ns)) {
      ...
      if (k == 1 & i == 2) {
        monthly_diffs %>%
          ggsave(filename = paste0("figs/final_figures/corrected_predicted_ice_extent_map_monthly_", models[j], "_", rcps[i], "_", ns[k], ".", device),
                 device = device,
                 units = "mm",
                 width = output_width,
      ...
      monthly_diffs %>%
        ggsave(filename = paste0("figs/final_figures/corrected_predicted_ice_extent_map_monthly_", models[j], "_", rcps[i], "_", ns[k], ".png"),
               device = "png",
               units = "mm",
               width = output_width,
               height = output_height,
...
for (j in 1:length(models)) {
  for (i in 1:length(rcps)) {
    for (k in 1:length(ns)) {
      ...
      if (ns[k] == "nh") {
        world_clipped = world_clipped_nh_land %>% st_union
        crs = 3995
        xlim = c(-7e06, 8e06)
        ylim = c(-6e06, 7e06)
        if (i == 1) {
        ...
        world_clipped = world_clipped_sh_land %>% st_union
        crs = 3031
        xlim = c(-7.5e6, 7.5e6)
        ylim = c(-7.5e6, 7.5e6)
        device = "tiff"
      ...
      duration_comps %>% 
        ggsave(filename = paste0("figs/final_figures/corrected_ice_duration_map_comparison_", models[j], "_", rcps[i], "_", ns[k], ".", device),
               device = device,
               units = "mm",
               width = 120,
               height = 70)
      ...
      duration_comps %>% 
        ggsave(filename = paste0("figs/final_figures/corrected_ice_duration_map_comparison_", models[j], "_", rcps[i], "_", ns[k], ".png"),
               # device = device,
               units = "in",
               width = 6,
               height = 3.5)
...
for (j in 1:length(models)) {
  for (i in 1:length(rcps)) {
    for (k in 1:length(ns)) {
      ...
      duration_diff %>% 
        ggsave(filename = paste0("figs/final_figures/corrected_ice_duration_map_difference_", models[j], "_", rcps[i], "_", ns[k], ".pdf"),
               device = "pdf",
               units = "mm",
               width = 60,
               height = 70,
...
predicted_ice_affected_percent_global_temp = annual_dat_global %>% 
  ...
  labs(
    ...
    y = "Percent of rivers affected by ice",
    color = "RCP",
    pch = "Model"
  ) +
...
predicted_ice_duration_global_temp = annual_dat_global %>% 
  ggplot() +
  geom_point(aes(x = delta_gsat, y = mean_ice_duration, pch = model, color = rcp, fill = rcp), size = 0.7) +
  ...
  labs(
    ...
    # scale_shape_manual(values = c("CESM1-BGC" = 15, "GFDL-ESM2M" = 17, "MIROC-ESM" = 21), guide = guide_legend(title.position = "top", direction = "vertical", keyheight = h, keywidth = w)) +
  scale_shape_discrete(guide = guide_legend(title.position = "top", direction = "vertical", keyheight = h, keywidth = w)) +
  scale_color_discrete(guide = guide_legend(title.position = "top", direction = "vertical", keyheight = h, keywidth = w)) +
  guides(fill = F) +
  theme(panel.grid.major.y = element_line(color = "lightgrey", size = 0.5, linetype = 1),
...
predicted_ice_duration_global_temp %>% 
  ggsave(
    filename = "figs/final_figures/corrected_predicted_ice_duration_global_delta_gsat.pdf",
    units = "mm",
    dpi = 300,
    colormode = "rgb",
    width = 60,
